...
grep -Fq 'pulseaudio-utils' "$INSTALL_SCRIPT" || fail "audio diagnostics are installed"
grep -Fq 'pactl list short sinks' "$INSTALL_SCRIPT" || fail "audio outputs are checked"
