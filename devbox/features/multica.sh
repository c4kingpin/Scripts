...
install_multica_daemon_service() {
  ...
  # Keep the generated helper identifiable on the box. This is particularly
  # useful when diagnosing an update: the helper, manager and root-state
  # version must all originate from the same DevBox release. Preserve the
  # shebang as the first line: systemd executes this file directly.
  printf '%s\n' '#!/usr/bin/env bash' >"$MULTICA_DAEMON_START_SCRIPT"
  printf '# Generated by DevBox %s; do not edit.\n' "$DEVBOX_VERSION" >>"$MULTICA_DAEMON_START_SCRIPT"

  cat <<'EOF' >>"$MULTICA_DAEMON_START_SCRIPT"
set -Eeuo pipefail

multica_has_configured_token() {
  # Do not use `multica auth status` here. It validates the token against the
  # API and can return "Not authenticated" during boot while a configured,
  # valid token is already on disk. The daemon performs the authoritative
  # server-side validation itself; this guard only distinguishes a box that
  # has never logged in from one that needs a retry.
  jq -e '.token | strings | length > 0' "$HOME/.multica/config.json" >/dev/null 2>&1
}
...
# A box that was never logged in is intentionally idle. Once a token is
# present, start the daemon directly and let it perform the API validation.
# Any transient API/DNS/proxy failure exits non-zero and systemd retries it.
if ! multica_has_configured_token; then
  echo "Multica has no configured token; nothing to start."
  exit 0
fi
