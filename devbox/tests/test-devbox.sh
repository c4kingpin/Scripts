...
multica_external_reverse_proxy_is_restricted_and_uses_token_login() {
  ...
  grep -Fq 'DEVBOX_MULTICA_APP_URL' "$FEATURE_MULTICA" &&
    ...
    grep -Fq 'configure_multica_github' "$FEATURE_MULTICA" &&
    grep -Fq 'DEVBOX_MULTICA_SMTP_PASSWORD' "$FEATURE_MULTICA" &&
    grep -Fq 'DEVBOX_MULTICA_GITHUB_APP_PRIVATE_KEY' "$FEATURE_MULTICA" &&
    grep -Fq 'GITHUB_APP_PRIVATE_KEY="${escaped_private_key}"' "$FEATURE_MULTICA" &&
    ...
    grep -Fq 'daemon status --output json' "$MANAGER" &&
    grep -Fq '"status": "running"' "$MANAGER" &&
...
multica_daemon_waits_for_backend_at_boot() {
  ...
  grep -Fq "printf '%s\\n' '#!/usr/bin/env bash' >\"\$MULTICA_DAEMON_START_SCRIPT\"" "$FEATURE_MULTICA" &&
    grep -Fq "printf '# Generated by DevBox %s; do not edit.\\n' \"\$DEVBOX_VERSION\"" "$FEATURE_MULTICA" &&
    ...
    grep -Fq 'jq -e' <<<"$start_script" &&
    grep -Fq 'Multica has no configured token; nothing to start.' <<<"$start_script" &&
    ! grep -Fq 'auth_status="$(multica auth status' <<<"$start_script" &&
...
interactive_multica_setup_can_collect_reverse_proxy_configuration() {
  grep -Fq 'DEVBOX_REMOTE_INTERACTIVE_SELECTION=0' "$INSTALL_SCRIPT" &&
    ...
    grep -Fq 'select_multica_github() {' "$INSTALL_SCRIPT" &&
    grep -Fq 'GitHub App private-key PEM' "$INSTALL_SCRIPT" &&
    grep -Fq 'select_multica_reverse_proxy' "$INSTALL_SCRIPT"
}
